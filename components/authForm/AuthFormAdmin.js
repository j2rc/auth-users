  ...
  */
  ...
  const handleSubmit = async (e) => {
    ...
    if (typeAuth == "signin") {
      ...
      res = await signIn("credentials-admin", { ...fields, redirect: false })
      ...
      try {        
        res = await axios({
          url: `${process.env.BASE_URL}/api/auth/admin/users/signup`,
          method: "post",
          data: fields
        })

        if (res.status == 201) {
          push(`${process.env.BASE_URL}/auth/admin/verify/${res.data._id}`)
        }
